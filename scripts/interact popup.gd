...
func _process(delta):
	if player_in_chest and Input.is_action_just_pressed("ui_select"):
		object.chest_open()
	if player_in_NPC and Input.is_action_just_pressed("ui_select") and !typing:
		...
		if current_dialogue.size() > 0:
			if current_line_index < current_dialogue.size():
				...
				$"dialogue panel".hide()
				player_in_NPC = false
	elif player_in_NPC and Input.is_action_just_pressed("ui_select") and typing:
			stop_typing = true
...
func _on_interact_area_area_entered(area):
	...
	elif area.has_method("chest_idle"):
		$"interact panel".show()
